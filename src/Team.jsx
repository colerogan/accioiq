...
function Team() {
  ...
  return (
    <section id="team" style={{ background: 'var(--aq-parchment)', padding: isMobile ? '72px 20px' : '128px 40px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          gap: isMobile ? 0 : 56,
          borderTop: '1px solid var(--aq-rule)', paddingTop: 40,
        }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {partners.map((p, i) => (
              <button key={p.name}
                ...
                <div style={{
                  width: 64, height: 64, background: active === i ? 'var(--aq-deep)' : 'var(--aq-parchment)',
                  border: '1px solid var(--aq-rule)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 400,
                  color: active === i ? 'var(--aq-teal)' : 'var(--aq-slate)',
                  transition: 'all 320ms var(--ease-out)', flexShrink: 0,
                }}>{p.initials}</div>
                <div style={{ flex: 1 }}>
                  <div style={{
                    fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 24,
                    color: active === i ? 'var(--aq-deep)' : 'var(--aq-slate)', lineHeight: 1.15,
                  }}>{p.name}</div>
                  <div className="aq-eyebrow" style={{
          ...
          <div style={{
            ...
            <div className="aq-eyebrow" style={{ color: 'var(--aq-teal-dark)' }}>
              {partners[active].role}
            </div>
            <div style={{
              fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: isMobile ? 30 : 40,
              lineHeight: 1.1, color: 'var(--aq-deep)', letterSpacing: '-0.01em',
            }}>{partners[active].name}</div>
            <p style={{
            ...
            }}>{partners[active].bio}</p>
            {partners[active].stats && (
              <div style={{
                display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0,
                marginTop: 16, borderTop: '1px solid var(--aq-rule)',
              }}>
                {partners[active].stats.map((s, i) => (
                  <div key={s.n} style={{
                    padding: isMobile ? '16px 12px 8px' : '24px 20px 8px',
                    borderRight: i < 2 ? '1px solid var(--aq-rule)' : 'none',
                  }}>
                    <div style={{
                      fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: isMobile ? 22 : 32,
                      color: 'var(--aq-teal-dark)', letterSpacing: '-0.01em',
                    }}>{s.n}</div>
                    <div style={{
                      fontFamily: 'var(--font-body)', fontWeight: 300, fontSize: 12,
                      color: 'var(--aq-slate)', marginTop: 8, lineHeight: 1.5,
                    }}>{s.b}</div>
                  </div>
                ))}
              </div>
